...
class BaseScraper(ABC):
    ...
    def download_file(self, url: str, local_path: str) -> bool:
        ...
        def _download() -> None:
            ...
            with httpx.stream("GET", url, headers=headers, timeout=config.scraper.timeout, follow_redirects=True) as response:
        ...
        try:
            self.logger.info(f"Downloading file", url=url, path=local_path)
            _download()
            self.logger.info(f"File downloaded successfully", path=local_path)
            return True
