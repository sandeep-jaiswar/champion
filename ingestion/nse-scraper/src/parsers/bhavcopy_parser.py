...
class BhavcopyParser:
    ...
    def parse(self, file_path: Path, trade_date: date) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Parsed bhavcopy file", path=str(file_path), events=len(events))
        return events
    ...
    def _build_payload(self, row: Dict[str, str]) -> Dict[str, Any]:
        ...
        def safe_float(value: str) -> float:
            ...
            try:
                return float(value.strip()) if value and value.strip() and value.strip() != '-' else None
        ...
        def safe_int(value: str) -> int:
            ...
            try:
                return int(float(value.strip())) if value and value.strip() and value.strip() != '-' else None
            except (ValueError, AttributeError):
                return None
