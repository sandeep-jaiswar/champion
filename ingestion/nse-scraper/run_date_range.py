...
def run_etl_for_date(
    ...
    try:
        ...
        output_file = parser.write_parquet(
            df=normalized_df, trade_date=target_date, base_path=output_base
        )
        file_size_mb = output_file.stat().st_size / (1024 * 1024)
        logger.info(
            "write_success", date=str(target_date), file=str(output_file), size_mb=file_size_mb
        )
        ...
        if load_clickhouse and clickhouse_config:
            try:
                ...
                logger.info("clickhouse_load_success", date=str(target_date), rows=len(data))
                metrics.clickhouse_load_success.labels(
                    table=clickhouse_config.get("table", "normalized_equity_ohlc")
                ).inc()
                result["clickhouse_loaded"] = True

            except Exception as ch_error:
                logger.error("clickhouse_load_failed", date=str(target_date), error=str(ch_error))
                metrics.clickhouse_load_failed.labels(
                    table=clickhouse_config.get("table", "normalized_equity_ohlc")
                ).inc()
...
Examples:
  ...
  poetry run python run_date_range.py --start-date 2024-01-02 --end-date 2024-01-05 --output /custom/path
    ...
    parser.add_argument(
        "--start-date", required=True, help="Start date in YYYY-MM-DD format (e.g., 2024-01-02)"
    )
    parser.add_argument(
        "--end-date", required=True, help="End date in YYYY-MM-DD format (e.g., 2024-01-31)"
    )
    parser.add_argument("--skip-weekends", action="store_true", help="Skip Saturday and Sunday")
    parser.add_argument(
        "--output",
        default="../../data/lake",
    ...
    parser.add_argument(
        "--clickhouse-host", default="localhost", help="ClickHouse host (default: localhost)"
    )
    parser.add_argument(
        "--clickhouse-port", type=int, default=8123, help="ClickHouse HTTP port (default: 8123)"
    )
    parser.add_argument(
        "--clickhouse-user", default="default", help="ClickHouse user (default: default)"
    )
    parser.add_argument("--clickhouse-password", default="", help="ClickHouse password")
    parser.add_argument(
        "--clickhouse-database", default="default", help="ClickHouse database (default: default)"
    )
    parser.add_argument(
        "--clickhouse-table",
    ...
    parser.add_argument(
        "--no-metrics", action="store_true", help="Disable Prometheus metrics server"
    )
    ...
    if args.load_clickhouse:
        print(
            f"🗄️  ClickHouse: {args.clickhouse_host}:{args.clickhouse_port}/{args.clickhouse_database}"
        )
    if not args.no_metrics:
        ...
        if result["status"] == "success":
            ...
            if args.load_clickhouse:
                if result.get("clickhouse_loaded"):
                    ...
                    ch_status = (
                        f" | ClickHouse ❌ ({result.get('clickhouse_error', 'unknown error')})"
                    )
    ...
    print(
        f"     poetry run python -c \"import polars as pl; df = pl.read_parquet('{output_base}/normalized/**/*.parquet'); print(df)\""
    )
    print("  3. Load to ClickHouse:")
    print("     cd ../../warehouse && poetry run python -m loader.batch_loader")
