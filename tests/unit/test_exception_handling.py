...
class TestExceptionHandling:
    ...
    def test_load_bulk_block_deals_clickhouse_file_not_found(self, caplog):
        ...
        assert any("parquet_read_failed" in record.message or 
                   "retryable" in str(record) 
    ...
    def test_parse_bse_polars_file_not_found(self, mock_parser_class, caplog):
        ...
        # Check that the error was logged with retryable flag
        assert any("bse_file_read_failed" in record.message or 
                   "retryable" in str(record) 
                   for record in caplog.records)
    ...
    def test_parse_bse_polars_validation_error(self, mock_parser_class, caplog):
        ...
        assert any("bse_parsing_validation_failed" in record.message or 
                   "retryable" in str(record) 
                   for record in caplog.records)
    ...
    def test_load_bulk_block_deals_value_error(self, mock_read_parquet, caplog):
        ...
        assert any("parquet_invalid_format" in record.message or 
                   "retryable" in str(record) 
                   for record in caplog.records)
...
class TestConversionFunctions:
    ...
    def test_to_int_conversion_with_valid_values(self):
        ...
        from champion.orchestration.tasks.bulk_block_deals_tasks import parse_bulk_block_deals
