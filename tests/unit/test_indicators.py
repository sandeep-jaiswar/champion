...
from champion.features.indicators import compute_sma, compute_ema, compute_rsi, compute_features
...
def test_compute_rsi_trending_up():
    ...
    df = pl.DataFrame({
        "symbol": ["TEST"] * 30,
        "trade_date": [date(2024, 1, i+1) for i in range(30)],
        "close": [100.0 + i * 2.0 + (i % 3 - 1) * 0.5 for i in range(30)],  # Rising with small variations
    })
