...
1. **HighValidationFailureRate** (Warning) - >5% failures for 5 minutes
2. **CriticalValidationFailureRate** (Critical) - >20% failures for 2 minutes

#### Pipeline Alerts

1. **PipelineDurationAnomaly** (Warning) - 2x average duration for 5 minutes
2. **PipelineFailureRate** (Critical) - >10% failure rate for 5 minutes

#### Warehouse Alerts

1. **WarehouseLoadFailure** (Critical) - Any load failures for 2 minutes
2. **WarehouseLoadHighLatency** (Warning) - p95 >60 seconds for 5 minutes
3. **ParquetWriteFailures** (Critical) - Any write failures for 2 minutes

#### Resource Alerts

1. **HighMemoryUsage** (Warning) - >85% of 4GB for 5 minutes
2. **CriticalMemoryUsage** (Critical) - >95% of 4GB for 2 minutes
3. **HighCPUUsage** (Warning) - >1.7 cores for 10 minutes
4. **ServiceDown** (Critical) - Service unreachable for 1 minute

#### Data Quality Alerts

1. **NoDataIngested** (Warning) - No ingestion during market hours for 30 minutes
2. **LowDataVolume** (Warning) - <100 stocks/hour for 30 minutes
